...
## Demographics
```{r message=FALSE, echo=FALSE, warning=FALSE, fig.height=6, fig.position="H", fig.cap="\\label{demo_table}"}
library(tidyverse)
library(kableExtra)
...
In addition, we include effective sample size diagnostics ($n_{\mathrm{eff}}$) for our our single-outcome, two-outcome, and all-outcome models in Supplementary Figures \ref{sv_neff}, \ref{2v_neff}, and \ref{mv_neff}.
...
```{r message=FALSE, echo=FALSE, warning=FALSE, fig.height=4, fig.width = 8, fig.cap="Convergence diagnostic for MCMC fitting procedure for the single outcome model. Box plots show effective sample sizes ($n_{\\mathrm{eff}}$) for each of the variables in the model. Color indicates sample groups, which were fit using separate models. Better models have $n_{\\mathrm{eff}}$ close to 2000, the nominal sample size. For variable definitions, see (\\ref{rating}) - (\\ref{nu}). \\label{sv_neff}"}
library(tidyverse)
library(kableExtra)
...
plt <- ggplot(effects) + 
  geom_boxplot(aes(y=n_eff, x=variable, color=group)) +
  group_color_scale +
...
```{r message=FALSE, echo=FALSE, warning=FALSE, fig.height=4, fig.width = 8, fig.cap="Convergence diagnostic for MCMC fitting procedure for the two-outcome model. Box plots show effective sample sizes ($n_{\\mathrm{eff}}$) for each of the variables in the model. Color indicates sample groups, which were fit using separate models. Better models have $n_{\\mathrm{eff}}$ close to 2000, the nominal sample size. For variable definitions, see (\\ref{rating_m}) - (\\ref{nu_m}). \\label{2v_neff}"}
library(tidyverse)
library(kableExtra)
...
```{r message=FALSE, echo=FALSE, warning=FALSE, fig.height=4, fig.width = 8, fig.cap="Convergence diagnostic for MCMC fitting procedure for the multi-outcome model. Box plots show effective sample sizes ($n_{\\mathrm{eff}}$) for each of the variables in the model. Color indicates sample groups, which were fit using separate models. Better models have $n_{\\mathrm{eff}}$ close to 2000, the nominal sample size. For variable definitions, see (\\ref{rating_m}) - (\\ref{nu_m}). \\label{mv_neff}"}
library(tidyverse)
library(kableExtra)
