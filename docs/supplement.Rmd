...
for (dname in mturk_sets) {
  ...
  dlist[[length(dlist) + 1]] <- dat %>% distinct(hashedID, .keep_all=TRUE)
}
...
mturk_demos$gender = factor(mturk_demos$gender, labels=c("Male", "Female", "Other"))
mturk_demos$race = factor(mturk_demos$race, labels=c("American Indian or Alaska Native", 
                                                     "Asian", 
...
mturk_demos$ethnicity = factor(mturk_demos$ethnicity, labels=c("Hispanic or Latino", 
...
mturk_demos$education = factor(mturk_demos$education, 
                               labels=c("Less than high school", 
                                        ...
                                        "Formal legal training (J.D., LL.M., J.S.D.) or licensed to practice law in country of citizenship", 
...
get_count_tbl <- function(varname){
  ...
  names(tbl) <- tools::toTitleCase(names(tbl))
  return(tbl)
}

kable(get_count_tbl("gender"))
kable(get_count_tbl("age"))
kable(get_count_tbl("race"))
kable(get_count_tbl("ethnicity"))
kable(get_count_tbl("education"))
kable(get_count_tbl("political_party"))
```
...
for (dname in mturk_sets) {
  ...
  dlist[[length(dlist) + 1]] <- dat
}

mturk_ratings <- bind_rows(dlist)
...
load('../data/stan_model_output_hier_t_mturkwith_demos.rdata')
